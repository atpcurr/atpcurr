...
:- dynamic(pathlim/0), dynamic(lit/6).
:- dynamic(option/1).
...
prove2(F,Set,Proof) :-
    ...
    retractall(lit(_,_,_,_,_,_)), (member([-(#)],M) -> S=conj ; S=pos),
    assert_clauses(M,S),
    retractall(option(_)),
...
prove(PathLim,Proof) :-
    ...
    lit(#,_,C,_,_,_) -> prove(C,[-(#)],PathLim,[],Proof1),
    Proof=[C|Proof1].
prove(PathLim,Proof) :-
...
assert_clauses([C|M],Set) :-
    ...
    copy_term(C1,C2), numbervars(C2,0,_), C2=Key, %term_hash(C2, Key),
    term_variables(C1, Vars),
    (ground(C) -> G=g ; G=n), assert_clauses2(C1,[],G, Key, Vars),
    assert_clauses(M,Set).

assert_clauses2([],_,_,_,_).
assert_clauses2([L|C],C1,G,Key, Vars) :-
    assert_renvar([L],[L2]), append(C1,C,C2), append(C1,[L],C3),
    assert_no_duplicate(lit(L2,L,C2,G,Key,Vars)), assert_clauses2(C,C3,G,Key, Vars).
...
assert_no_duplicate(lit(L2,L,C2,G,Key,Vars)):-
    Contra = [L|C2], copy_term(Contra, ContraX), numbervars(ContraX),
    ( lit(_,LY,CY,_,_,_), ContraY=[LY|CY], numbervars(ContraY), ContraX == ContraY -> true
    ; assert(lit(L2,L,C2,G,Key,Vars))
    ).
...
select_unifiable_contras(E, Contras):-
    findall( ext(NegL,Cla1,Grnd1,Key,Vars), (
                 lit(E,NegL,Cla1,Grnd1,Key,Vars),
...
select_paramodulation(Lit, Paramodulations):-
    findall(para(Pos, LHS, RHS, Cla1, Dir, Key, Vars), (
                                             lit(-(_=_), -(LHS=RHS), Cla1, _Grnd1,Key,Vars),
                                             \+ LHS == RHS,
