...
logic_embed(StateOrig,FHash,EmbType, EmbStateP,EmbStateV,EmbActions):-
    ...
    ( mc_param(collapse_vars,1) -> collapse_vars(State)
    ; numbervars(State, 1000, _VarCount)
    ),
...
varStats(Goals, NumVar, NumVarOcc, NumVarOneOcc, NumVarMoreOcc, MostOcc, LeastOcc):-
    ...
    countVars(VarOccList, 0, 0, 0, 0, 0, 10000, NumVar, NumVarOcc, NumVarOneOcc, NumVarMoreOcc, MostOcc, LeastOcc0),
    ( LeastOcc0 = 10000 -> LeastOcc = 0 ; LeastOcc = LeastOcc0 ).
