        ...
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                with tf.variable_scope("loss", reuse=False):
                    ...
                    # klmcts = self.policy_pi.mcts_distribution.kl(self.policy_pi.proba_distribution)
                    klmcts = self.policy_pi.proba_distribution.kl(self.policy_pi.mcts_distribution)
                    meanklmcts = tf.reduce_mean(klmcts)
                    pol_mctspen = 0 * self.entcoeff * meanklmcts
                    ...
                    total_loss = pol_surr + pol_entpen + vf_loss + pol_mctspen
                    losses = [pol_surr, pol_entpen, vf_loss, pol_mctspen, meankl, meanent, meanklmcts]
                    self.loss_names = ["pol_surr", "pol_entpen", "vf_loss", "pol_mctspen", "kl", "ent", "klmcts"]

                    tf.summary.scalar('entropy_loss', pol_entpen)
                    tf.summary.scalar('mcts_loss', pol_mctspen)
                    tf.summary.scalar('policy_gradient_loss', pol_surr)
                    tf.summary.scalar('value_function_loss', vf_loss)
                    tf.summary.scalar('approximate_kullback-leibler', meankl)
                    tf.summary.scalar('mcts_approximate_kullback-leibler', meanklmcts)
